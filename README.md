...
### Инструкция по установке (в том числе для чистой Lubuntu/Ubuntu 16.04)
Если вы устанавливаете этот пакет на свежую или "голую" систему (особенно старую, вроде 16.04), вам потребуется предварительно установить системные пакеты. Без них приложение не установится из-за нехватки библиотек Qt.

> [!IMPORTANT]
> Выполните все команды шаг за шагом в терминале (`Ctrl+Alt+T`) строго до попытки установить сам `.deb` файл.

**Шаг 1: Обновление списков пакетов**
Сначала нужно дать системе понять, какие пакеты доступны в интернете:
```bash
sudo apt update
```

**Шаг 2: Установка системных зависимостей**
Установим необходимые пакеты (графический движок, на котором написан интерфейс приложения):
```bash
sudo apt install -y python3-pyqt5
```

**Шаг 3: Установка самого приложения**
Теперь, когда все зависимости удовлетворены, можно смело ставить наш пакет:
```bash
sudo dpkg -i telnet-gui_1.2.3.deb
```

#### Что делать, если вы уже попытались поставить пакет и получили ошибку?
Если вы сначала запустили `sudo dpkg -i telnet-gui_1.2.3.deb`, и система выдала ошибку нехватки пакета `python3-pyqt5`, не пугайтесь! Пакетный менеджер просто "подвис" в состоянии прерванной установки.

Чтобы всё автоматически починить и докачать, просто введите:
```bash
sudo apt install -f -y
```
Система сама скачает недостающий PyQt5 и автоматически завершит установку нашего приложения!
